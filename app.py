...
# ================== CONFIG ==================
SYMBOL = "ETH/USDT"
RISK_PERCENT = 0.008           # 0.8% risk for small balance
...
state = {
    ...
    "leverage": LEVERAGE,
    "mode": "FUTURES",
    "logs": ["v68.7 - Dashboard Mode Toggle + Small Balance Fix"],
    "history": [] 
}
...
def get_usdt_balance():
    try:
        ...
        params = {'type': 'swap'} if state["mode"] == "FUTURES" else {}
        balance = exchange.fetch_balance(params=params)
        usdt = balance.get('total', {}).get('USDT') or balance.get('USDT', {}).get('free', 0) or 0
        return float(usdt)
    except:
        return 0.0
...
def execute_trade(side):
    ...
    try:
        ...
        if usdt_balance < MIN_ORDER_USDT:
            add_log(f"❌ Balance too low: ${usdt_balance:.2f} (need ≥ ${MIN_ORDER_USDT})")
            return

        if side == 'buy' and not state["active_position"]:
            risk_amount = usdt_balance * RISK_PERCENT
            cost = max(risk_amount * 12, MIN_ORDER_USDT + 3)   # Bigger position for futures with small capital
            ...
            add_log(f"✅ LONG OPENED ${filled_cost:.2f} → {filled_amount:.6f} ETH")
    ...
    while True:
        try:
            ...
            if daily_pnl < -(usdt_balance * MAX_DAILY_LOSS_PCT) and not state["is_paused"]:
                ...
                add_log("🚨 DAILY LOSS LIMIT - PAUSED")
...
async def start_engine(request: Request):
    ...
    try:
        ...
        state["status"] = f"LIVE - {state['mode']} MODE RUNNING"
        add_log(f"✅ Started in {state['mode']} mode | Balance: ${balance:.2f} USDT")
    except Exception as e:
...
def home():
    return """
    <!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Alpha v68.7</title>
    <style>
        ...
        .stat-box h4 { margin: 0; color: #848e9c; font-size: 10px; }
        .stat-box h2 { margin: 6px 0 0 0; font-size: 16px; }
        .logs { background: #000; color: #0ecb81; padding: 15px; font-family: monospace; font-size: 11px; height: 300px; overflow-y: auto; border-radius: 8px; }
    </style></head>
    <body>
        <div class="container">
            <div>
                <div class="card">
                    <div class="header"><span style="font-weight:bold; font-size:18px;">ALPHA v68.7</span><span id="mode_display" style="color:var(--yellow)"></span></div>
                    <div style="margin-bottom:15px;"><span>Status: </span><b id="status">OFFLINE</b></div>
                    <select id="mode_select">
                        <option value="FUTURES">Futures (Recommended)</option>
                ...
                <div class="card"><h3 style="margin:0 0 15px 0; color:#848e9c;">SYSTEM TERMINAL</h3><div class="logs" id="logs"></div></div>
            </div>
            <div style="display:flex; flex-direction:column; gap:15px;">
                <div class="grid-stats">
                    <div class="stat-box"><h4>ETH Price</h4><h2 id="price" style="color:var(--yellow)">--</h2></div>
                    <div class="stat-box"><h4>RSI</h4><h2 id="rsi">--</h2></div>
                ...
                <div class="card"><div style="padding:40px 10px; text-align:center; border:2px solid var(--border); border-radius:12px;" id="sig_area">
                    <div style="color:#848e9c; font-size:12px;">MARKET ANALYSIS</div>
                    <div style="font-size:34px; font-weight:900;" id="signal">STANDBY</div>
                    <div id="active_pos" style="margin-top:20px;"></div>
                </div></div>
                <div class="card"><h3 style="margin:0 0 15px 0; color:#848e9c;">TRADE HISTORY</h3><div style="max-height:200px; overflow-y:auto;">
                    <table><thead><tr><th>Time</th><th>Action</th><th>Price</th><th>PnL</th></tr></thead><tbody id="history"></tbody></table>
                </div></div>
            </div>
        </div>
        <script>
            ...
            async function update() {
                ...
                if (d.signal.includes("LONG") || d.signal.includes("BUY")) {
                    sig.style.color = "#0ecb81";
                } else if (d.signal.includes("CLOSE")) {
                    sig.style.color = "#f6465d";
                }

                const logsEl = document.getElementById('logs');
                logsEl.innerHTML = d.logs.map(l => `<div>${l}</div>`).join("");

                if (d.active_position) {
                    document.getElementById('active_pos').innerHTML = `POSITION: \( {d.active_position.amount} ETH<br>PnL: <b> \){d.active_position.current_pnl}</b>`;
                } else {
